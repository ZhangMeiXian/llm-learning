# 偏好对齐学习之DPO完整实现，论文地址：https://arxiv.org/abs/2305.18290

# 大模型训练通常通过以下步骤：预训练->指令微调->RHLF强化学习或DPO偏好对齐学习
# RHLF需要单独训练一个奖励模型，成本较高，而DPO则是利用最大似然估计设计损失函数直接在模型基础上实现偏好对齐，成本较低
import json
...
test_data = data[train_portion:train_portion + test_portion]  # 55
val_data = data[train_portion + test_portion:]  # 110
...
policy_model.to(device)  # 用于偏好对齐的微调模型
reference_model.to(device)  # 保持不变的基线模型
...
logits = torch.tensor(
    [[2.0, 1.0, 0.1], [0.5, 2.5, 0.3]]  # Shape: (2, 3)
)
...
selected_log_probs = torch.gather(
    input=log_softmax_logits, dim=-1, index=targets.unsqueeze(-1)
).squeeze(-1)
manual_loss = -selected_log_probs.mean()  # Average over the batch

# PyTorch loss
crossentropy_loss = F.cross_entropy(logits, targets)

assert manual_loss == crossentropy_loss
...
def evaluate_dpo_loss_loader(policy_model, reference_model, train_loader, val_loader, beta, eval_iter):
    ...
    with torch.no_grad():
        train_loss, train_chosen_rewards, train_rejected_rewards = compute_dpo_loss_loader(
            data_loader=train_loader, policy_model=policy_model, reference_model=reference_model, beta=beta,
            num_batches=eval_iter
        )
        val_loss, val_chosen_rewards, val_rejected_rewards = compute_dpo_loss_loader(
            data_loader=val_loader, policy_model=policy_model, reference_model=reference_model, beta=beta,
            num_batches=eval_iter
        )
...
def train_model_dpo_simple(policy_model, reference_model, train_loader, val_loader, optimizer, num_epochs, beta,
